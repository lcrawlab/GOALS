...
mat ComputeKFast(arma::mat X,arma::mat B, int j, double h = 1){
    ...
    const int p = X.n_cols;
    vec onevec = ones(n);
    mat Cj = B%exp(-(h/p)*(1-2*(X.col(j)*trans(onevec)-onevec*trans(X.col(j)))));
    return Cj;
}

// [[Rcpp::export]]
mat ComputeDeltaVar(arma::mat X, arma::mat B, arma::mat Ainv){
	int i;
	const int n = X.n_rows;
	const int p = X.n_cols;
	vec onevec = ones(n);
	mat var_delta = zeros(p,1);
	mat BAinv = B*Ainv;
	mat kappa_mat = (trans(onevec)*(B - BAinv*B)*onevec);
	float kappa = kappa_mat[0,0];
#pragma omp parallel for schedule(dynamic)
	for (i=0; i<p; i++){
		mat C = ComputeKFast(X, B, i);
		mat alpha_mat = trans(onevec)*(B - trans(C)*Ainv*C)*onevec;
		float alpha = alpha_mat[0,0];
		mat gamma_mat = trans(onevec)*(C - BAinv*C)*onevec;
		float gamma = gamma_mat[0,0];
	    var_delta[i] = (kappa+alpha-2*gamma)/pow(n,2);
	}
	return var_delta;
}

// [[Rcpp::export]]
float AlphaCalc(arma::mat C, arma::mat Ainv, arma::mat B){
	const int n = C.n_rows;
	vec onevec = ones(n);
	mat alpha_mat = trans(onevec)*(B - trans(C)*Ainv*C)*onevec;
	return alpha_mat[0,0];
}

// [[Rcpp::export]]
float GammaCalc(arma::mat C, arma::mat Ainv, arma::mat B){
	const int n = C.n_rows;
	vec onevec = ones(n);
	mat alpha_mat = trans(onevec)*(C- trans(B)*Ainv*C)*onevec;
	return alpha_mat[0,0];
}

// [[Rcpp::export]]
float var_calc(float kappa, float gamma, float alpha, int n){
	return (kappa+alpha-2*gamma)/pow(n,2);
}

// [[Rcpp::export]]
float KappaCalc(arma::mat X, arma::mat B, arma::mat Ainv){
	const int n = X.n_rows;
	vec onevec = ones(n);
	mat BAinv = B*Ainv;
	mat kappa = trans(onevec)*(B-BAinv*B)*onevec;
	return kappa[0,0];
}
