...
RATE = function(X = X, f.draws = NULL,beta.draws = NULL,prop.var = 1, low.rank = FALSE, rank.r = min(nrow(X),ncol(X)), nullify = NULL,snp.nms = snp.nms, cores = 1){
  ...
  KLD = foreach(j = int, .combine='c', .export = 'sherman_r')%dopar%{
    q = unique(c(j,l))
    m = abs(mu[q])
